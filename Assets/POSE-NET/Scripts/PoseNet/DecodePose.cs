...
public partial class PoseNet
{
    ...
    Vector2 GetDisplacementOG(int edgeId, Vector2Int point, Tensor displacements) {

        var numEdges = (int)(displacements.shape[3] / 2);
        ...
        float[,,,] displacementsBwd)
    {
        ...
        Keypoint TraverseToTargetKeypointOG(
        int edgeId, Keypoint sourceKeypoint, int targetKeypointId,
        Tensor scores, Tensor offsets, int outputStride,
        Tensor displacements) {

        var height = scores.shape[1];
        var width = scores.shape[2];
        ...
        var displacement =
            GetDisplacementOG(edgeId, sourceKeypointIndices, displacements);
        ...
        var offsetPoint = GetOffsetPointOG(
                displacedPointIndices.y, displacedPointIndices.x, targetKeypointId,
                offsets);
    ...
    Keypoint[] DecodePoseOG(PartWithScore root, Tensor scores, Tensor offsets,
        int outputStride, Tensor displacementsFwd,
        Tensor displacementsBwd)
    {
        
        var numParts = scores.shape[3];
        var numEdges = parentToChildEdges.Length;
        ...
        var rootPart = root.part;
        var rootScore = root.score;
        var rootPoint = GetImageCoordsOG(rootPart, outputStride, offsets);
        ...
        for (var edge = numEdges - 1; edge >= 0; --edge)
        {
            ...
            if (instanceKeypoints[sourceKeypointId].score > 0.0f &&
                instanceKeypoints[targetKeypointId].score == 0.0f)
            {
                instanceKeypoints[targetKeypointId] = TraverseToTargetKeypointOG(
                    edge, instanceKeypoints[sourceKeypointId], targetKeypointId, scores,
                    offsets, outputStride, displacementsBwd);
            }
        }
